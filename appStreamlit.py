...
def mostrar_segmentacion_con_leyenda(image, mask, classes):
    ...
    if file:
        if file.type.startswith("image"):
            ...
            with st.spinner("Segmentando imagen..."):
                mask, duration = segment_image(image)

            st.markdown(f"**Tiempo de inferencia:** {duration:.3f} segundos")
            mostrar_segmentacion_con_leyenda(image, mask, classes)
            ...
            while True:
                ...
                pred_mask, _ = segment_frame(frame)
